	...
	for _, voter := range poll.GetVoters() {
		...
		case hasVotedIncorrectly:
			...
			v.keeper.Logger(ctx).Debug("penalized voter due to incorrect vote",
				...
				"poll", poll.GetKey().String())
		case hasVotedLate:
			v.keeper.Logger(ctx).Debug("held rewards for voter due to late vote",
				"voter", voter.String(),
				"poll", poll.GetKey().String())
		case hasVoted:
...
func (v voteHandler) HandleResult(ctx sdk.Context, result codec.ProtoMarshaler) error {
	...
	err = handleEvents(cacheCtx, chainK, v.nexus, v.signer, voteEvents.Events, chain)
	if err != nil {
...
func NewVoteHandler(cdc codec.Codec, keeper types.BaseKeeper, nexus types.Nexus, signer types.Signer, rewarder types.Rewarder) vote.VoteHandler {
	return voteHandler{
		cdc:      cdc,
		keeper:   keeper,
		nexus:    nexus,
